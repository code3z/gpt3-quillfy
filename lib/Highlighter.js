...
class InstantSearch {
  ...
  acceptSuggestion(suggestion) {
    ...
    element
      ...
      .forEach((el) => {
        ...
        const newChild = document.createTextNode(suggestion)
    ...
    if (parent && parent.childNodes) {
      Array.from(parent?.childNodes).reduce((acc, val) => {
        if (val instanceof Element) {
          if (acc && normalize(acc.tagName) === normalize(val.tagName)) {
  ...
  search(node) {
    ...
    const tokenStr = caseSensitive ? token.text : normalize(token.text)

    for (let i = 0; i < text.length; i++) {
      const char = text[i]
      const next = `${state.current || ""}${
        caseSensitive ? char : normalize(char)
      }`.replace(/\s+/g, " ")
      if (next === tokenStr) {
  ...
  wrapRange(range, className, startNode, endNode) {
    ...
    marker.style.cssText =
      "background-color: #ff009e; opacity:0.3; border-bottom: 1px solid yellow; pointer-events: all; visibility: visible !important; color: transparent !important;"

    if (this.eventListener) marker.addEventListener("click", this.eventListener)
  ...
  removeEmptyElement(element, originalNode) {
    const isInOriginalNode = (element) =>
